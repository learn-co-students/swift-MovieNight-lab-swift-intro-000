...
extension BasicMovieView {
    ...
    func clearContentsForReUse() {
        ...
        moviePosterImageView.image = #imageLiteral(resourceName: "Nothing")
    }
...
extension BasicMovieView: MovieImageDelegate {
    ...
    func imageUpdate(withMovie movie: Movie) {
        ...
        case .NoImage(let image):
            queue.addOperation(duration: 0.6, self.moviePosterImageView.image = image)
        case .downloaded(let image):
            ...
            queue.addOperation(animation: randomAnimation, duration: 0.8, self.moviePosterImageView.image = image)
        case .nothing:
            let image = #imageLiteral(resourceName: "Nothing")
            queue.addOperation(animation: .transitionCrossDissolve, duration: 0.1, self.moviePosterImageView.image = image)
        }
    }
